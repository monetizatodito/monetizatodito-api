    ...
    try {
      const { rows } = await pool.query<{
        ...
        SELECT
          ...
          t.slug AS ar_slug,
          t.titulo AS ar_title,
          t.descripcion AS ar_description,
          t.contenido AS ar_content
        FROM blog b
      ...
      for (const row of rows) {
        const slug = typeof row.ar_slug === "string" ? row.ar_slug.trim() : "";
        if (!slug) continue;

        const link = `${SITE}/ar/blog/${encodeURIComponent(slug)}`;

        const titleText = stripHtml(row.ar_title) || "مقال بدون عنوان";
        const title = xmlEscape(titleText);

        const descriptionText =
          typeof row.ar_description === "string"
            ? row.ar_description.trim()
            : stripHtml(row.ar_description);

        const contentText = stripHtml(row.ar_content);

        const rawDescription =
          descriptionText ||
          contentText.slice(0, 220) ||
          "اقرأ هذا المقال في Mosan Multiverso";
      ...
      console.error("Error generando RSS AR:", error);
      res.status(500).send("Error generando RSS AR");
    }
  }
}

export default new RssARXmlControlador();
